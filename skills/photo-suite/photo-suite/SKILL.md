---
name: photo-suite
description: Generate consistent 4-photo sets using a model card for face-locking. Use this skill when moom asks for photo sets, beauty shots, outfit photos, or套图. Supports reference images for face, environment, and outfit.
version: 0.5.0
requires_toolsets:
  - code_execution
  - skills
...
Before anything else, put together a short creative brief and present it to moom for approval. Use web search to check current trending topics if relevant.
...
│   └── <session-id>/
│       ├── grid.png         # Step 3 output
│       ├── crop-1.png       # Step 5 output
│       ├── crop-2.png
│       ├── crop-3.png
│       └── crop-4.png
└── logs/
...
- All images are 9:16 portrait aspect ratio, 2x2 grid layout
- Default resolution is 4K — cropped images are ~2K, sufficient for most uses
