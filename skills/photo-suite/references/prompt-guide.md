...
wanw 在执行 photo-suite 流程时的提示词参考手册。
...
| 便利店/小卖部场景不真实 | 完全禁止文字会让店面失去“店”的感觉；允许清晰文字又会暴露 AI | 不要写成 `no text anywhere`。改成“有招牌/标签感，但文字必须小、远、虚焦、局部遮挡、不可完整读清”，例如 `subtle Chinese signage present but not fully legible`, `small blurred packaging labels`, `no clearly readable brand names or words` |
| 连拍第二张像上一张的裁切或超分 | 只要求“更近、更清楚”，没有换动作/叙事/机位 | 在下一张 prompt 里明确换“微剧情”与动作，例如“刚买完雪糕回头”“走进店里停一下”，并改变构图类型（全身→三分之二身 / 半身），不要只做 tighter crop |
| 全身照里脸显得有点糊 | 2K 分辨率下脸在画面占比太小，生成模型会把脸部细节柔化 | 若要脸更清楚，改成半身或 3/4 身，让脸在画面中占比更大；同时单独强调 `crisp eyes`, `defined lip edges`, `natural strands of hair`, `realistic skin texture` |

## Chain Mode 实战补充

### 1. 不要把“连续性”写成“重复构图”
Chain mode 的目标是锁住人、衣服、气氛，但每一张都应该像新的镜头，而不是上一张的裁切版。保持连续性的同时，主动更换：
- 动作（走路 / 回头 / 买东西 / 停一下）
- 机位（全身 / 半身 / 近景）
- 场景落点（门口 / 店内 / 沙滩 / 栈道）
- 情绪层级（自然 / 暧昧 / 危险感）
...
### 2. 便利店 / 小卖部题材的最稳写法
这类场景里，最好把“店味”交给这些元素，而不是可读文字：
- 暖色门灯 / 冰柜灯
- 货架轮廓与包装块面
- 一支无字冰饮 / 雪糕
- 店门外能看到海、椰树、夜色

这样既像中国海岛小卖部，又不容易被 AI 文字拖垮。

### 3. 如果用户说“自然，但脸有点糊”
说明当前图的气氛是对的，问题在细节分配。优先策略不是重做同一张，而是：
- 保留当前气氛和穿搭
- 把下一张改成更近的人像构图
- 让脸占比更大
- 同时引入一个新的小动作，避免变成裁切感

这样通常能同时保住“自然感”和“脸部清晰度”。
